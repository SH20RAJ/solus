...
export default function LocationPage() {
	...
	const filteredPosts = posts.filter(
		(post) => post.location?.toLowerCase() === decodedLocation.toLowerCase()
	);
	...
	return (
		<div className="max-w-[640px] mx-auto py-8 px-4 select-none animate-slide-up">
			...
			<header className="mb-8 flex items-center justify-between">
				<div>
					<h1 className="text-2xl font-bold tracking-tight text-text-primary font-serif flex items-center gap-2">
						<span>📍</span> {decodedLocation}
					</h1>
					<p className="mt-1 text-xs text-text-muted">
						{filteredPosts.length} {filteredPosts.length === 1 ? "memory" : "memories"} tagged here
					</p>
				</div>
				<Link
					href="/home"
