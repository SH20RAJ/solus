...
export default function CreatePageClient() {
	...
	const uploadFileHelper = async (uploadFile: File): Promise<string> => {
		...
		return presignData.data.uploadUrl;
	};
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		if (entryType === "journal" && !journalTitle.trim()) {
		...
		try {
			if (entryType === "journal") {
				// Handle Journal cover file upload
				let coverUrl: string | undefined;
				if (coverFile) {
					coverUrl = await uploadFileHelper(coverFile);
				}

				// Create journal (stored in collection endpoint mapping to journey table)
				...
				if (file) {
					...
					mediaUrl = await uploadFileHelper(file);
				}
		...
		return (
			<div className="py-16 sm:py-24 px-4 sm:px-6 w-full max-w-[500px] mx-auto text-center animate-fade-in font-sans">
				...
				<h1 className="text-2xl font-semibold tracking-tight text-text-primary font-serif">
					{entryType === "journal" ? "Journal Created" : "Sealed into your archive"}
				</h1>
				<p className="text-xs text-text-muted mt-2">
					{entryType === "journal"
						? "Your new book of memories is ready."
				...
				<button
					type="button"
					onClick={() => router.push(entryType === "journal" ? "/profile" : "/home")}
	...
	return (
		<div className="py-6 sm:py-12 px-4 sm:px-6 w-full max-w-[620px] mx-auto animate-slide-up select-none font-sans">
			...
			{/* Post/Story/Journal Toggle */}
			<div className="p-1 rounded-xl bg-card border border-border/20 mb-8 max-w-md flex items-center shadow-inner">
				<button
					type="button"
					onClick={() => {
						setEntryType("post");
			...
			<form onSubmit={handleSubmit} className="space-y-8">
				{/* JOURNAL CREATOR FORM */}
				{entryType === "journal" ? (
					<div className="space-y-6 text-left">
						{/* Title */}
						<div className="space-y-2">
							<label htmlFor="journalTitle" className="block text-[10px] uppercase tracking-wider font-mono text-text-muted">
								Journal Book Title
							</label>
							<input
								id="journalTitle"
								...
								placeholder="e.g. Tokyo Wanderings 2026"
								required
								className="w-full h-11 px-4 rounded-[14px] border border-border/40 bg-card text-text-primary text-xs placeholder:text-text-muted/50 focus:outline-none focus:ring-1 focus:ring-accent/40 focus:border-accent transition-all duration-300"
							/>
						</div>
						...
						<div className="space-y-2">
							<label className="block text-[10px] uppercase tracking-wider font-mono text-text-muted">
								Journal Introduction / Description
							</label>
							<TiptapEditor onChange={setJournalDesc} placeholder="Write a summary or layout details for this journal..." />
						</div>
					</div>
				) : (
					/* STANDARD POST & STORY FORM */
					<>
				...
				<button
					type="submit"
					disabled={isSubmitting || (entryType === "journal" ? !journalTitle.trim() : (!file && !caption))}
